...
public class amazonData {
	...
	public static void main(String[] args) {
		...
		HashMap<String, ArrayList<Rating> > ratingsMap = readFile("./src/Books.csv");
		if (ratingsMap == null) {
			System.out.println("Map could not be created");
			System.exit(-1);
		}
	...
	private static void doStuffBucketed(
		...
		*/
		...
		for (Entry<String, ArrayList<ArrayList<Rating>>> entry : ratingsMapBucket.entrySet()) {
			...
			// Only print for users which have at least 3 ratings in exp level 5, which 
			// means they have at least 15 ratings total
			if (bucketedRatings.get(4).size() < 3){
				totalCount++;
				continue;
			}
			System.out.println("User: " + entry.getKey());
			...
			for (int i = 0; i < NUM_EXPERIENCE_LEVEL; i++) {
				...
				if (ratings.size() == 0) {
					...
					Rating r = ratings.get(0);
					System.out.println("Starts at time: " + sdf.format(r.getTime()));
					if (i == 0) timestamp = r.getTime();
					if (i == NUM_EXPERIENCE_LEVEL-1){
						if (timestamp.equals(r.getTime())){
							sameDayCount += ratings.size();
						}else{
							difDayCount += ratings.size();
						}
					}
				}
			}
		...
		System.out.println("Users with many reviews: " + manyReviewsCount);
		System.out.println("Counts on the same day: " + sameDayCount);
		System.out.println("Counts on dif days: " + difDayCount);
